...
# <p>Векторы обычно обозначаются строчными буквами, например v. Существует несколько способов записи векторов:</p>
# <ul>
#     <li><strong>Строчная запись:</strong> v = (v₁, v₂, v₃)
#         <br>где v₁, v₂, v₃ - скалярные величины, обычно действительные числа в пространстве R³
#     </li>
...
# <p>В декартовой системе координат векторы характеризуются следующими понятиями:</p>
# <ul>
...
#             <li>Предпочтительно использовать термин "ортогональные" вместо "перпендикулярные"</li>
#             <li>Обеспечивает независимость координат</li>
#         </ul>
#     </li>
# </ul>
#
# <p><em>Примечание:</em> Координатные векторы являются базисом для построения любых других векторов в заданной системе координат.</p>
#
# <h4>Орты и базис</h4>
...
#         <br><em>Пояснение: вектор идёт на 3 единицы вправо (вдоль i) и на 2 единицы вверх (вдоль j)</em>
#     </li>
#     <li>Вектор b = (2, 0) = 2i + 0j
#         <br><em>Пояснение: вектор направлен только вдоль оси i на 2 единицы</em>
#     </li>
#     <li>Вектор c = (0, -5) = 0i - 5j
#         <br><em>Пояснение: вектор направлен только вдоль оси j вниз на 5 единиц</em>
#     </li>
# </ul>
#
# <p><strong>Важные наблюдения:</strong></p>
# <ul>
#     <li>Каждый вектор однозначно определяется своими координатами в базисе (i,j)</li>
...
#     <li>Отрицательные координаты означают направление в противоположную сторону от соответствующего орта</li>
# </ul>
#
# <p><em>Примечание: Такое представление векторов через базисные векторы позволяет легко выполнять операции над векторами, используя их координаты.</em></p>
#
...
#         <br>d = (i, -4j)
#         <br>e = (-3i, -3j)
#         <br><em>Пояснение: запись через координаты при соответствующих ортах</em>
#     </li>
#     <li>Второй способ (алгебраическая запись):
#         <br>d = i + (-4j)
#         <br>e = -3(i + j)
#         <br><em>Пояснение: запись через сумму произведений координат на орты</em>
#     </li>
# </ul>
#
...
# </ul>
#
# <p><em>Примечание: Все эти способы записи эквивалентны и используются в зависимости от контекста задачи.</em></p>
#
...
#         <br>Вектор, который можно переносить параллельно самому себе в любую точку пространства
#         <br>Характеризуется:
#         <ul>
#             <li>Длиной</li>
#             <li>Направлением</li>
#             <li>Не зависит от точки приложения</li>
#             <li>Все параллельные переносы вектора считаются одним и тем же вектором</li>
#         </ul>
#     </li>
...
#         <br>Упорядоченный набор чисел, с которым можно производить алгебраические операции:
#         <ul>
#             <li>Сложение</li>
#             <li>Вычитание</li>
...
# <p>Два вектора можно сложить, получив третий вектор. Обозначается: c = a + b</p>
#
# <p>Правила сложения:</p>
# <ul>
#     <li>Новый вектор имеет ту же размерность, что и исходные векторы</li>
#     <li>Каждый элемент нового вектора получается сложением соответствующих элементов исходных векторов</li>
...
# <p>Рассмотрим два произвольных ненулевых вектора a и b. Для нахождения их суммы:</p>
# <ol>
#     <li>Так как векторы свободные, можно перенести вектор b, отложив его от конца вектора a</li>
#     <li>Сумма векторов c = a + b - это вектор, идущий из начала вектора a в конец вектора b</li>
# </ol>
#
# <p><strong>Важные замечания:</strong></p>
# <ul>
#     <li>При сложении векторов используется свойство свободных векторов - возможность параллельного переноса</li>
#     <li>Результирующий вектор не зависит от выбора начальной точки</li>
#     <li>Длина суммы векторов может быть меньше суммы длин слагаемых векторов</li>
# </ul>
#
...
#     <li>Достроить фигуру до параллелограмма, проведя дополнительные линии параллельно данным векторам</li>
...
# <ol>
#     <li>Векторы располагаются последовательно, начало каждого следующего вектора совпадает с концом предыдущего</li>
#     <li>Сумма векторов - вектор, идущий из начала первого вектора в конец последнего</li>
# </ol>
#
...
# </ul>
#
# <p><em>Примечание:</em> Оба способа (параллелограмм и ломаная) дают одинаковый результат.</p>
#
# <h3 id="programming">2.4 Представление в программировании</h3>
# <p>Двумерный массив — это структура данных, которая представляет собой таблицу, состоящую из строк и столбцов.</p>
#
# <p><strong>Особенности представления векторов в программировании:</strong></p>
# <ul>
...
# <p>Два вектора называются коллинеарными, если они лежат на одной прямой или на параллельных прямых.</p>
#
...
#         <br>Если два ненулевых вектора коллинеарны и направлены противоположно
#         <br>Характеристики:
#         <ul>
#             <li>Лежат на параллельных прямых или одной прямой</li>
...
#         <br>Вектор e получается из вектора a умножением на √3 (корень из трех)
#         <br>Векторы e и a сонаправлены, так как √3 > 0
#         <br>Длина вектора e в √3 раз больше длины вектора a
#     </li>
#     <li><strong>b = (1/2)a</strong>
#         <br>Вектор b получается из вектора a умножением на 1/2 (то есть b в два раза короче a)
...
# <h2 id="broadcasting">4. Броадкастинг (Broadcasting) в векторных операциях</h2>
#
# <h3 id="broadcasting-definition">4.1 Определение броадкастинга</h3>
# <p>Броадкастинг - это механизм, который позволяет выполнять арифметические операции между массивами разных размерностей или между массивом и скаляром.</p>
#
# <p><strong>Основные принципы броадкастинга:</strong></p>
# <ul>
#     <li>Автоматическое расширение размерности меньшего массива до размерности большего</li>
...
# <ol>
#     <li>Если массивы имеют разное количество измерений, форма массива с меньшей размерностью дополняется единицами слева</li>
#     <li>Если форма массивов в каком-то измерении не совпадает, массив с размером 1 в этом измерении растягивается до большего размера</li>
# </ol>
#
# <h3 id="broadcasting-advantages">4.4 Преимущества броадкастинга</h3>
# <ul>
#     <li><strong>Эффективность памяти:</strong> Не требуется создавать копии данных</li>
#     <li><strong>Производительность:</strong> Оптимизированные векторные операции</li>
#     <li><strong>Читаемость кода:</strong> Упрощает написание векторизованных операций</li>
#     <li><strong>Гибкость:</strong> Позволяет работать с данными разных размерностей</li>
# </ul>
#
...
#
# <p>Броадкастинг - это мощный механизм в NumPy и других библиотеках для работы с векторами и матрицами, который предоставляет следующие преимущества:</p>
#
# <ul>
#     <li><strong>Гибкость операций:</strong> Возможность выполнять операции между массивами разных размерностей</li>
#     <li><strong>Оптимизация памяти:</strong> Экономия памяти за счёт отсутствия необходимости создавать временные массивы</li>
#     <li><strong>Чистота кода:</strong> Возможность писать более чистый и понятный код</li>
#     <li><strong>Производительность:</strong> Оптимизация вычислений на уровне библиотеки</li>
# </ul>
#
# <h4 id="broadcasting-rules-importance">Важность понимания правил броадкастинга</h4>
#
# <p>Глубокое понимание правил броадкастинга необходимо для:</p>
#
# <ul>
#     <li><strong>Предотвращения ошибок:</strong> Избегание проблем при работе с массивами разных размерностей</li>
#     <li><strong>Оптимизации:</strong> Эффективное использование векторизованных операций</li>
#     <li><strong>Проектирования:</strong> Правильное проектирование алгоритмов обработки данных</li>
# </ul>

# <h2>Разложение вектора по базису</h2>
#
# <div class="vector-points">
#     <p>Заданные точки:</p>
#     <ul>
#         <li>a(3, 2)</li>
#         <li>b(2, 0)</li>
...
# </div>
#
# <div class="vector-basis">
#     <p>Базисные векторы:</p>
#     <ul>
#         <li>i = 1·i + 0·j</li>
#         <li>j = 0·i + 1·j</li>
#     </ul>
#     <p>Координаты базисных векторов:</p>
#     <ul>
#         <li>i(1, 0)</li>
#         <li>j(0, 1)</li>
#     </ul>
# </div>
#
# <div class="basis-definition">
#     <h3>Разложение вектора по базису</h3>
#     <p>Разложение вектора по базису - это представление произвольного вектора в виде линейной комбинации базисных векторов:</p>
#     <p class="formula">a = x·i + y·j + z·k</p>
#     <p>где:</p>
#     <ul>
#         <li>x, y, z - координаты вектора</li>
#         <li>i, j, k - базисные векторы</li>
#     </ul>
#     <p>Свойства разложения:</p>
#     <ul>
#         <li>Разложение единственно для данного базиса</li>
#         <li>Коэффициенты разложения являются координатами вектора в данном базисе</li>
#         <li>Базисные векторы линейно независимы</li>
#     </ul>
# </div>
#
# <div class="definition">
#     <p>У вектора есть компоненты и мы раскладываем их по векторам.</p>
#     <p>Это называется разложение вектора по базису.</p>
# </div>
#
# <div class="example">
#     <h3>Пример разложения:</h3>
#     <p>Для вектора a(2, 3, 4):</p>
#     <p class="formula">a = 2i + 3j + 4k</p>
#     <p>где числа 2, 3, 4 являются координатами вектора в стандартном базисе</p>
# </div>
...
# Точка для демонстрации
point_a = [3, 2, 4]  # точка a(3,2,4)

# Отображаем точку a
ax.scatter(point_a[0], point_a[1], point_a[2], color="purple", s=100)
point_label = f"  a({point_a[0]},{point_a[1]},{point_a[2]})"
ax.text(
    point_a[0],
    point_a[1],
    point_a[2],
...
# Орты (единичные векторы базиса)
# Орт i
ax.quiver(0, 0, 0, 1, 0, 0, color="red", arrow_length_ratio=0.1)
ax.scatter(1, 0, 0, color="red", s=100)
ax.text(1.1, 0, 0, "i(1,0,0)", fontsize=10)

# Орт j
ax.quiver(0, 0, 0, 0, 1, 0, color="green", arrow_length_ratio=0.1)
ax.scatter(0, 1, 0, color="green", s=100)
ax.text(0, 1.1, 0, "j(0,1,0)", fontsize=10)

# Орт k
ax.quiver(0, 0, 0, 0, 0, 1, color="blue", arrow_length_ratio=0.1)
ax.scatter(0, 0, 1, color="blue", s=100)
ax.text(0, 0, 1.1, "k(0,0,1)", fontsize=10)
...
ax.text(point_a[0] / 2, -0.5, 0, f"{point_a[0]}i", color="red", fontsize=12)
...
# Формула разложения
formula = (
    f"a = {point_a[0]}i + {point_a[1]}j + {point_a[2]}k\n"
    "i(1,0,0), j(0,1,0), k(0,0,1) - орты"
)
ax.text(
    2,
    4,
...
plt.title("Разложение вектора a по ортам i,j,k")
plt.show()
